...
function ImportExport() {
  ...
  const handleExport = async () => {
    ...
    const deckCardIds = new Set(
      cards.map((card) => card.id).filter(Boolean) as number[],
    );
  ...
  const parsePayload = (payload: unknown) => {
    ...
    if (Array.isArray(payload)) {
      ...
      const container = payload as Partial<ExportPayload> & {
        cards?: ImportCard[];
      };
  ...
  const runImport = async (payload: unknown) => {
    ...
    if (insertedCards === 0) {
      const reason =
        errors.length > 0 ? errors[0] : t("importExport.noValidCards");
  ...
  const parseCsvTsv = (text: string, delimiter: string): unknown => {
    ...
    const hasFront =
      firstLine.includes("front") || firstLine.includes("front_md");
    const hasBack = firstLine.includes("back") || firstLine.includes("back_md");
    ...
    if (hasFront && hasBack) {
      ...
      // Positional: col 0 = front, col 1 = back, col 2 = tags (optional)
      headers = ["front_md", "back_md", "tags"];
      dataLines = lines;
    }

    const frontIdx =
      headers.indexOf("front_md") !== -1
        ? headers.indexOf("front_md")
        : headers.indexOf("front");
    const backIdx =
      headers.indexOf("back_md") !== -1
        ? headers.indexOf("back_md")
        : headers.indexOf("back");
    const tagsIdx = headers.indexOf("tags");

    if (frontIdx === -1 || backIdx === -1) {
      throw new Error(t("importExport.csvUnsupported"));
    }
      ...
      .map((line) => {
        ...
        const tags = rawTags
          ? rawTags
              ...
              .filter(Boolean)
          : [];
        return { front_md: front, back_md: back, tags };
      })
      .filter((card) => card.front_md && card.back_md);
  };
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus(
        t("importExport.importError", { message: (error as Error).message }),
      );
    }
  };
    ...
    <main className="container page">
      ...
      <section>
        ...
        <input
          id="import-json"
          type="file"
          accept="application/json,.csv,.tsv"
