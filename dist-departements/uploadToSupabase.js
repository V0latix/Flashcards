...
export async function uploadAllDepartementSvgs() {
    ...
    if (bucketsRes.error)
        throw new Error(`listBuckets failed: ${bucketsRes.error.message}`);
    ...
    if (!bucketExists) {
        ...
        if (createRes.error)
            throw new Error(`createBucket(${BUCKET}) failed: ${createRes.error.message}`);
    }
    ...
    await Promise.all(files.map((file) => limit(async () => {
        ...
        await withRetry(async () => {
            ...
            if (res.error)
                throw new Error(`upload ${path} failed: ${res.error.message}`);
        }, 5);
        uploaded += 1;
        if (uploaded % 20 === 0) {
