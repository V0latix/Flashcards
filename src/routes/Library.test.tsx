...
describe("Library delete by tag", () => {
  ...
  it("removes cards for selected tag including sub-tags", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: /Supprimer toutes les cartes de ce tag/i,
      }),
    );
  ...
  it("exports only cards from current selected tag", async () => {
    ...
    await screen.findByText(/Bibliothèque/i);
    const geographieTag = await screen.findByRole("button", {
      name: /Geographie/i,
    });
    fireEvent.click(geographieTag);
    fireEvent.click(
      screen.getByRole("button", { name: /Exporter la sélection/i }),
    );
    ...
    expect(payload.cards.map((card) => card.front_md)).toEqual(
  ...
  it("suspends a card and excludes it from training queue", async () => {
    ...
    fireEvent.click(
      within(q1Card as HTMLElement).getByRole("button", {
        name: /Suspendre la carte/i,
      }),
    );
    ...
    await waitFor(() => {
      expect(
        within(q1Card as HTMLElement).getByRole("button", {
          name: /Réactiver la carte/i,
        }),
      ).toBeInTheDocument();
    });

    await waitFor(async () => {
      const suspendedCard = (await db.cards.toArray()).find(
        (card) => card.front_md === "Q1",
      );
      const state = suspendedCard?.id
        ? await db.reviewStates.get(suspendedCard.id)
        : undefined;
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Session d'entraînement/i }),
    );
  ...
  it("filters cards by suspended status", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /^Suspendue \(\d+\)$/i }),
    );
  ...
  it("suspends and resumes all cards from current filter", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Suspendre la sélection/i }),
    );
    ...
    await waitFor(async () => {
      ...
      expect(
        geographieStates.every(
          (state) => state?.box === 0 && state?.due_date === null,
        ),
      ).toBe(true);
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Réactiver la sélection/i }),
    );
