...
const loadSessionCards = async (
  ...
  const cardById = new Map(
    cards.filter((card) => !card.suspended).map((card) => [card.id, card]),
  );
...
export async function autoFillBox1(
  ...
  await db.transaction("rw", db.cards, db.reviewStates, async () => {
    ...
    const box1Cards = await loadCardsByIds(
      box1States.map((state) => state.card_id),
    );
    const box0Cards = await loadCardsByIds(
      box0States.map((state) => state.card_id),
    );
    ...
    const box0StatesByCardId = new Map(
      box0States.map((state) => [state.card_id, state]),
    );
...
export const hasPendingDailyCards = async (
  todayInput: string,
): Promise<boolean> => {
  const today = normalizeTodayKey(todayInput);
  const { learnedReviewIntervalDays } = getLeitnerSettings();
  const [reviewStates, cards] = await Promise.all([
...
type ApplyReviewResultSnapshot = {
...
export const applyReviewResult = async (
  ...
  todayInput: string,
  wasReversed = false,
): Promise<ApplyReviewResultSnapshot> => {
  const today = normalizeTodayKey(todayInput);
  const { intervalDays } = getLeitnerSettings();
  let snapshot: ApplyReviewResultSnapshot | null = null;
  ...
  await db.transaction("rw", db.reviewStates, db.reviewLogs, async () => {
    ...
    snapshot = { clientEventId, previousState: { ...reviewState } };
  });
...
export const revertReviewResult = async (
  _cardId: number,
  previousState: ReviewState,
  clientEventId: string,
): Promise<void> => {
  await db.transaction("rw", db.reviewStates, db.reviewLogs, async () => {
    ...
    await db.reviewLogs.where("client_event_id").equals(clientEventId).delete();
  });
  markLocalChange();
};
