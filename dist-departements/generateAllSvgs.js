...
export async function generateAllDepartementSvgs() {
    ...
    await Promise.all(existing.filter((f) => f.endsWith('.svg')).map((f) => unlink(join(OUT_SVG_DIR, f))));
    ...
    await writeFile(join(OUT_DEPARTEMENTS_DIR, 'departements.meta.json'), JSON.stringify({ generated_at: new Date().toISOString(), metas }, null, 2), 'utf8');
    await writeFile(join(OUT_DEPARTEMENTS_DIR, 'preview.html'), buildPreviewHtml(Object.keys(metas).sort()), 'utf8');
    return { generated };
}
function buildPreviewHtml(codes) {
    const items = codes
        .map((code) => `<a class="card" href="svg/${code}.svg" target="_blank" rel="noreferrer"><img loading="lazy" src="svg/${code}.svg" alt="${code}"/><div class="label">${code}</div></a>`)
        .join('');
    return `<!doctype html>
<html lang="fr">
  <head>
