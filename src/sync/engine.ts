...
// Safety overlap applied to the delta cursor to absorb clock skew between devices.
// Rows written within this window before lastSyncAt are re-fetched on each incremental sync.
...
const mapRemoteCardToLocal = (card: RemoteCard): Card => ({
  ...
  updated_at: card.updated_at,
  source_type:
    card.source_type === "public_pack" ? "supabase_public" : "manual",
...
const mergeSnapshots = async (
  ...
  const remoteProgressByCard = new Map(
    remote.progress.map((progress) => [progress.card_id, progress]),
  );
  const remoteLogIds = new Set(
    remote.reviewLogs.map((log) => log.client_event_id),
  );
  ...
  local.cards.forEach((card) => {
    ...
    // In delta mode, a card absent from the remote snapshot simply hasn't changed
    // since lastSyncAt — it was NOT necessarily deleted. Re-uploading it would
    // resurrect cards deleted on another device. Only upload truly new local cards
    // (those that have never been synced: synced_at is null).
    if (isDelta && card.synced_at) {
      return;
    }
    // Full snapshot: defensive upsert (card may be missing due to API row caps, not deletion).
  ...
  local.reviewStates.forEach((state) => {
    ...
    if (!remoteProgress) {
      // In delta mode, absence from snapshot means the state hasn't changed remotely.
      // Only upload if this state was modified locally since the last sync.
      if (
        isDelta &&
        state.updated_at &&
        parseTime(state.updated_at) <= lastSyncMs
      ) {
        return;
      }
      progressToUpsert.push(
        mapLocalProgressToRemote(userId, card.cloud_id, state),
      );
      return;
    }
    if (isRemoteNewer(remoteProgress.updated_at, state.updated_at)) {
      ...
      if (localCardId) {
        localProgressUpserts.push(
          mapRemoteProgressToLocal(localCardId, remoteProgress),
        );
      }
  ...
  remote.progress.forEach((remoteProgress) => {
    ...
    const localState = local.reviewStates.find(
      (state) => state.card_id === localCardId,
    );
    if (!localState) {
      localProgressUpserts.push(
        mapRemoteProgressToLocal(localCardId, remoteProgress),
  ...
  remote.reviewLogs.forEach((log) => {
    ...
    const exists = local.reviewLogs.some(
      (entry) => entry.client_event_id === log.client_event_id,
    );
    if (!exists) {
      ...
      saveSettingsFromRemote(
        {
          ...
          intervalDays: Object.fromEntries(
            Object.entries(remote.settings.intervals).map(([key, value]) => [
              Number(key),
              value,
            ]),
          ),
          learnedReviewIntervalDays:
            remote.settings.learned_review_interval_days,
...
const handleInitialSync = async (userId: string) => {
  ...
  if (remoteEmpty && !localEmpty) {
    ...
    const cards = local.cards
      .filter((card): card is Card & { cloud_id: string } =>
        Boolean(card.cloud_id),
      )
    ...
    await db.transaction(
      ...
      async () => {
        ...
        for (const progress of remote.progress) {
          const local = localCards.find(
            (entry) => entry.cloudId === progress.card_id,
          );
          if (!local) {
            continue;
          }
          await db.reviewStates.add(
            mapRemoteProgressToLocal(local.localId, progress),
          );
        }
        for (const log of remote.reviewLogs) {
          const local = localCards.find(
            (entry) => entry.cloudId === log.card_id,
          );
          if (!local) {
            continue;
          }
  ...
  await mergeSnapshots(userId, local, remote, false);
  setLastSyncAt(new Date().toISOString());
};
...
export const syncOnce = async (userId: string, forcePull = false) => {
  ...
  try {
    ...
    if (pendingDeletes.length > 0) {
      ...
      // Clear optimistically, but restore on failure so they are retried next sync.
      pendingDeletes = [];
      try {
        await deleteRemoteCards(userId, deletes);
      } catch (err) {
        pendingDeletes = [...new Set([...pendingDeletes, ...deletes])];
        throw err;
      }
    }
    // Apply a safety overlap to absorb clock skew: re-fetch rows from the last
    // DELTA_OVERLAP_MS window so boundary writes are never silently missed.
    const lastSyncAt = getLastSyncAt();
    const deltaSince = lastSyncAt
      ? new Date(parseTime(lastSyncAt) - DELTA_OVERLAP_MS).toISOString()
      : null;
    const isDelta = deltaSince !== null;
    const remote = await fetchRemoteSnapshot(userId, deltaSince);
    await mergeSnapshots(userId, local, remote, isDelta);
    setLastSyncAt(new Date().toISOString());
  } catch (error) {
