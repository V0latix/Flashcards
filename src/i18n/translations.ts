export type Language = "fr" | "en";

export const translations = {
  fr: {
    nav: {
      home: "Accueil",
      session: "Session",
      add: "Ajouter",
      library: "Bibliothèque",
      stats: "Stats",
      settings: "Paramètres",
    },
    search: {
      title: "Recherche globale",
      placeholder: "Chercher une carte…",
      noResults: "Aucune carte trouvée.",
      hint: "↑↓ naviguer · Entrée ouvrir · Échap fermer",
    },
    info: {
      aria: "Aide: flashcards et méthode de Leitner",
      title: "Comment ça marche ?",
      flashcard1:
        "Une flashcard est une carte de révision: une question (recto) et une réponse (verso).",
      flashcard2:
        "Tu t'auto-interroges, tu essaies de répondre de mémoire, puis tu vérifies la réponse. Ce rappel actif renforce l'apprentissage.",
      leitnerTitle: "Principe des boîtes de Leitner",
      leitner1:
        "Les cartes sont rangées par niveau. Plus une carte est maîtrisée, plus elle est revue tard.",
      box1: "Boîte 1: nouvelles cartes ou cartes ratées, revues très souvent.",
      box2: "Boîte 2: cartes partiellement acquises, revues régulièrement.",
      box3: "Boîte 3: cartes plutôt solides, revues à intervalles plus longs.",
      box4: "Boîte 4: cartes bien retenues, revues peu souvent.",
      box5: "Boîte 5: cartes maîtrisées; elles passent en maintien.",
      cadence:
        "Réponse correcte: la carte monte d'une boîte. Réponse incorrecte: elle redescend vers la boîte 1.",
      why: "Ce système concentre l’effort sur ce que tu oublies et espace les rappels sur ce que tu sais déjà, ce qui améliore la mémoire à long terme.",
    },
    streak: {
      label: "Streak",
      loading: "Chargement de la streak...",
      signedOut: "Connecte-toi pour suivre ta streak.",
      todayDone: "Aujourd'hui validé",
      todayPending: "Aujourd'hui non validé",
      error: "Streak indisponible",
    },
    actions: {
      addCard: "Ajouter une carte",
      import: "Importer",
      packs: "Packs",
      training: "Session d'entraînement",
      suspendCard: "Suspendre la carte",
      resumeCard: "Réactiver la carte",
      suspendSelection: "Suspendre la sélection",
      resumeSelection: "Réactiver la sélection",
      up: "Remonter",
      delete: "Supprimer",
      cancel: "Annuler",
      close: "Fermer",
      save: "Enregistrer",
      restoreDefaults: "Restaurer les valeurs par défaut",
      loadMore: "Charger plus",
    },
    labels: {
      search: "Recherche",
      today: "Aujourd'hui",
      date: "Date",
      tomorrow: "Demain",
      nextReview: "Prochain test",
      box: "Boîte",
      boxes: "Boîtes",
      total: "Total",
      card: "Carte",
      tags: "Tags",
      due: "À faire",
      suspended: "Suspendue",
      hint: "Indice",
      showHint: "Afficher l'indice",
      hideHint: "Masquer l'indice",
      deleteCard: "Supprimer",
      learnedReview: "Revue de maintien (jours)",
      reverseQA: "Inverser question/réponse",
      intervals: "Intervalles (jours)",
      appearance: "Apparence",
      darkMode: "Mode sombre",
      language: "Langue",
      includeSubTags: "Inclure les sous-tags",
    },
    status: {
      loading: "Chargement...",
      none: "—",
    },
    auth: {
      account: "Compte",
      login: "Connexion",
      loggedInAs: "Connecté en tant que {{email}}.",
      signOut: "Se déconnecter",
      signedOut: "Déconnecté.",
      githubCta: "Continuer avec GitHub",
      githubHint: "Connecte-toi avec GitHub pour synchroniser tes données.",
      or: "ou",
      emailLabel: "Email",
      emailPlaceholder: "ton@email.com",
      emailHint: "Reçois un lien magique de connexion par email.",
      emailCta: "Continuer avec email",
      emailInvalid: "Renseigne un email valide.",
      emailSent: "Lien de connexion envoyé à {{email}}.",
    },
    home: {
      startSession: "Commencer une session",
      startSessionDesc: "Lance la session complète du jour.",
      startBoxSession: "Démarrer la boîte {{box}}",
      startBoxSessionCta: "Réviser cette boîte",
      addDesc: "Carte, import JSON ou packs.",
      importJson: "Importer JSON",
      library: "Bibliothèque",
      libraryDesc: "Tes cartes locales et tags.",
      settingsDesc: "Paramètres Leitner.",
      statsDesc: "Progression et performances.",
    },
    library: {
      title: "Bibliothèque",
      subtitle: "Explore tes cartes par dossiers de tags.",
      allCards: "Toutes les cartes",
      tag: "Tag",
      noFront: "Sans recto",
      deleteByTag: "Supprimer toutes les cartes de ce tag",
      noTags: "Aucun tag pour le moment.",
      noCards: "Aucune carte pour le moment.",
      nextReview: "Prochain test",
      boxFilter: "Boîtes",
      clearBoxes: "Toutes",
      exportSelection: "Exporter la sélection",
      exportFailed: "Erreur export: {{message}}",
    },
    settings: {
      title: "Paramètres",
      subtitle:
        "Configurer la taille de la boîte 1 et les intervalles Leitner.",
      review: "Révision",
      box1Target: "Objectif boîte 1",
      learnedIntervalHelp: "Révision de maintien pour les cartes apprises.",
      dangerZone: "Zone dangereuse",
      dangerDesc:
        "Suppression totale des cartes et de toutes les données locales.",
      deleteAll: "Supprimer toutes les cartes",
      deleteAllTitle: "Suppression totale",
      deleteAllMessage:
        "Toutes les cartes, sessions, statistiques et progressions seront supprimées.",
      confirmDeleteToken: "SUPPRIMER",
      confirmDelete: 'Tapez "{{token}}" pour confirmer',
      defaultRestored: "Valeurs par défaut restaurées.",
      saved: "Paramètres enregistrés.",
      positiveNumbers: "Les valeurs doivent être des nombres positifs.",
      learnedIntervalBounds:
        "L'intervalle des cartes apprises doit être entre {{min}} et {{max}} jours.",
      deletedDone: "Suppression terminée.",
    },
    stats: {
      title: "Stats",
      subtitle: "Suivre tes progrès sur 7, 30 ou 90 jours.",
      overview: "Vue globale",
      progress: "Progression (révisions)",
      boxSplit: "Répartition par boîte",
      tagsAll: "Tous les tags",
      selectTag: "Sélectionne un tag pour voir les détails.",
      aggregates: "Agrégats",
      noData: "Aucune donnée.",
      workOn: "À travailler",
      lowRateHint: "Tags avec faible taux de réussite (min {{min}} révisions).",
      period7: "7 jours",
      period30: "30 jours",
      period90: "90 jours",
      dueToday: "À faire aujourd'hui",
      learned: "Apprises",
      reviewsToday: "Révisions aujourd'hui",
      successRate7d: "Réussite 7j",
      reviews: "Révisions",
      chartLegend: "Vert: {{good}}, rose: {{bad}}.",
      rate: "Taux",
      reviewTag: "Réviser ce tag",
      boxDistribution: "Distribution des boîtes",
      weakTags: "Tags à renforcer",
      heatmapTitle: "Activité (365 jours)",
      heatmapSummary: "{{total}} révisions sur {{days}} jours actifs.",
      heatmapLess: "Moins",
      heatmapMore: "Plus",
      reviewsUnit: "révisions",
      dow: {
        mon: "L",
        wed: "M",
        fri: "V",
      },
      retentionTitle: "Rétention par boîte",
      retentionSubtitle:
        "Taux de réussite des révisions, selon la boîte de départ.",
      retentionHint:
        "Un taux élevé en boîte 1 signifie que les nouvelles cartes sont bien acquises rapidement.",
    },
    markdown: {
      imageAlt: "Image",
      imageNotFound: "Image introuvable",
    },
    review: {
      title: "Session",
      empty: "Aucune carte à réviser.",
      startTraining: "Démarrer l'entraînement",
      trainingMode: "Mode entraînement (ne modifie pas la progression).",
      progress: "Progression",
      remaining: "{{count}} carte(s) restante(s)",
      cardSize: "Taille des cartes",
      revealBack: "Révéler le verso",
      completed: "Session terminée",
      trainingCards: "Cartes d'entraînement",
      noGood: "Aucune bonne réponse.",
      noBad: "Aucune mauvaise réponse.",
      backHome: "Retour à l'accueil",
      good: "BON",
      bad: "FAUX",
      editCard: "Modifier la carte",
      deleteCard: "Supprimer la carte",
      confirmDelete: "Supprimer définitivement cette carte ?",
      confirmDeleteYes: "Supprimer",
      confirmDeleteNo: "Annuler",
      undo: "Annuler la dernière réponse",
    },
    packs: {
      title: "Packs",
      subtitle: "Explorer et télécharger des packs depuis Supabase.",
      search: "Recherche",
      descriptionFallback: "Sans description.",
      cardsCount: "{{count}} carte(s)",
      importDirect: "Importer",
      importResult: "{{imported}} importées, {{already}} déjà présentes",
      importFailed: "Erreur import: {{message}}",
      download: "Télécharger",
      open: "Ouvrir",
      all: "Tous les packs",
      none: "Aucun pack.",
      browseByTags: "Parcours les packs publics par dossier de tags.",
    },
    importExport: {
      title: "Importer / Exporter",
      subtitle: "Importer ou exporter des cartes au format JSON.",
      importTitle: "Importer",
      exportTitle: "Exporter",
      pasteJson: "Coller le JSON ici",
      exportJson: "Exporter en JSON",
      exportInProgress: "Export en cours...",
      exportDone: "Export terminé.",
      importInProgress: "Import en cours...",
      schemaUnsupported: "Version de schéma non supportée",
      jsonUnsupported: "Format JSON non supporté",
      missingFrontBack: "Carte {{index}} : recto/verso manquant",
      duplicateId: "Carte {{index}} : ID dupliqué, nouvel ID généré",
      exportPool: "Exporter le pool",
      importPool: "Importer un pool",
      importSample: "Importer un échantillon (debug)",
      summary: "Résumé d'import",
      noValidCards: "Aucune carte valide",
      importNoInsert: "Import sans insertion : {{reason}}",
      importDone: "Import terminé.",
      importError: "Erreur import : {{message}}",
      importCsvTsv: "Importer un fichier CSV/TSV",
      csvUnsupported:
        "Fichier CSV/TSV invalide : colonnes front/back introuvables",
      exportAnki: "Exporter en Anki (.apkg)",
      importAnki: "Importer un fichier Anki (.apkg)",
      exportAnkiInProgress: "Export Anki en cours...",
      exportAnkiDone: "Export Anki terminé.",
      ankiImportDone: "Import Anki terminé : {{count}} cartes importées.",
      ankiImportEmpty: "Aucune carte valide trouvée dans le fichier Anki.",
      ankiImportError: "Erreur lors de l'import Anki : {{message}}",
    },
    packDetail: {
      title: "Pack",
      slug: "Slug",
      import: "Importer ce pack dans ma bibliothèque",
      importResult: "{{imported}} cartes importées, {{already}} déjà présentes",
      total: "Total",
      none: "Aucune carte.",
      backToPacks: "Retour aux packs",
    },
    cardEditor: {
      titleNew: "Nouvelle carte",
      titleEdit: "Modifier la carte",
      front: "Recto",
      back: "Verso",
      hint: "Indice (optionnel)",
      tags: "Tags (un tag par ligne, / pour la hiérarchie)",
      preview: "Aperçu",
      save: "Enregistrer",
      delete: "Supprimer",
    },
    sharedDeck: {
      import: "Importer dans ma bibliothèque",
      importResult:
        "{{imported}} carte(s) importée(s), {{already}} déjà présente(s).",
      importError: "Erreur d'import : {{message}}",
      notFound: "Ce deck partagé est introuvable ou a expiré.",
      cardCount: "{{count}} carte(s)",
      shareTitle: "Partager la sélection",
      shareTitleLabel: "Titre du deck partagé",
      shareConfirm: "Créer le lien",
      shareCancel: "Annuler",
      shareCreating: "Création en cours...",
      shareLinkLabel: "Lien de partage",
      shareCopy: "Copier",
      shareCopied: "Copié !",
      shareCopyError: "Impossible de copier. Copiez manuellement.",
      shareError: "Erreur lors du partage : {{message}}",
      shareLoginRequired: "Tu dois être connecté pour partager un deck.",
    },
    notifications: {
      title: "Notifications",
      subtitle: "Rappel quotidien si des cartes sont à réviser.",
      enable: "Activer les rappels",
      permissionDenied: "Permission refusée par le navigateur.",
      permissionGranted: "Notifications activées.",
    },
  },
  en: {
    nav: {
      home: "Home",
      session: "Session",
      add: "Add",
      library: "Library",
      stats: "Stats",
      settings: "Settings",
    },
    search: {
      title: "Global search",
      placeholder: "Search a card…",
      noResults: "No cards found.",
      hint: "↑↓ navigate · Enter open · Esc close",
    },
    info: {
      aria: "Help: flashcards and Leitner method",
      title: "How it works",
      flashcard1:
        "A flashcard is a study card: a question on the front and an answer on the back.",
      flashcard2:
        "You test yourself, try to answer from memory, then check the answer. This active recall strengthens learning.",
      leitnerTitle: "Leitner box method",
      leitner1:
        "Cards are grouped by level. The better a card is known, the less often it is reviewed.",
      box1: "Box 1: new or failed cards, reviewed very frequently.",
      box2: "Box 2: partly learned cards, reviewed regularly.",
      box3: "Box 3: fairly solid cards, reviewed at longer intervals.",
      box4: "Box 4: well retained cards, reviewed less often.",
      box5: "Box 5: mastered cards; they move to maintenance review.",
      cadence:
        "Correct answer: the card moves up one box. Incorrect answer: it drops back toward box 1.",
      why: "This system focuses effort on what you forget and spaces repetition for what you already know, which improves long-term memory.",
    },
    streak: {
      label: "Streak",
      loading: "Loading streak...",
      signedOut: "Sign in to track your streak.",
      todayDone: "Today completed",
      todayPending: "Today not completed",
      error: "Streak unavailable",
    },
    actions: {
      addCard: "Add a card",
      import: "Import",
      packs: "Packs",
      training: "Training session",
      suspendCard: "Suspend card",
      resumeCard: "Resume card",
      suspendSelection: "Suspend selection",
      resumeSelection: "Resume selection",
      up: "Up",
      delete: "Delete",
      cancel: "Cancel",
      close: "Close",
      save: "Save",
      restoreDefaults: "Restore defaults",
      loadMore: "Load more",
    },
    labels: {
      search: "Search",
      today: "Today",
      date: "Date",
      tomorrow: "Tomorrow",
      nextReview: "Next review",
      box: "Box",
      boxes: "Boxes",
      total: "Total",
      card: "Card",
      tags: "Tags",
      due: "Due",
      suspended: "Suspended",
      hint: "Hint",
      showHint: "Show hint",
      hideHint: "Hide hint",
      deleteCard: "Delete",
      learnedReview: "Maintenance review (days)",
      reverseQA: "Reverse question/answer",
      intervals: "Intervals (days)",
      appearance: "Appearance",
      darkMode: "Dark mode",
      language: "Language",
      includeSubTags: "Include sub-tags",
    },
    status: {
      loading: "Loading...",
      none: "—",
    },
    auth: {
      account: "Account",
      login: "Sign in",
      loggedInAs: "Signed in as {{email}}.",
      signOut: "Sign out",
      signedOut: "Signed out.",
      githubCta: "Continue with GitHub",
      githubHint: "Sign in with GitHub to sync your data.",
      or: "or",
      emailLabel: "Email",
      emailPlaceholder: "you@email.com",
      emailHint: "Get a magic sign-in link by email.",
      emailCta: "Continue with email",
      emailInvalid: "Enter a valid email address.",
      emailSent: "Sign-in link sent to {{email}}.",
    },
    home: {
      startSession: "Start a session",
      startSessionDesc: "Kick off today's full session.",
      startBoxSession: "Start box {{box}}",
      startBoxSessionCta: "Review this box",
      addDesc: "Card, JSON import, or packs.",
      importJson: "Import JSON",
      library: "Library",
      libraryDesc: "Your local cards and tags.",
      settingsDesc: "Leitner settings.",
      statsDesc: "Progress and performance.",
    },
    library: {
      title: "Library",
      subtitle: "Explore your cards by tag folders.",
      allCards: "All cards",
      tag: "Tag",
      noFront: "No front",
      deleteByTag: "Delete all cards from this tag",
      noTags: "No tags yet.",
      noCards: "No cards yet.",
      nextReview: "Next review",
      boxFilter: "Boxes",
      clearBoxes: "All",
      exportSelection: "Export selection",
      exportFailed: "Export error: {{message}}",
    },
    settings: {
      title: "Settings",
      subtitle: "Configure box 1 size and Leitner intervals.",
      review: "Review",
      box1Target: "Box 1 target",
      learnedIntervalHelp: "Maintenance review for learned cards.",
      dangerZone: "Danger zone",
      dangerDesc: "Full deletion of cards and local data.",
      deleteAll: "Delete all cards",
      deleteAllTitle: "Full deletion",
      deleteAllMessage:
        "All cards, sessions, stats, and progress will be deleted.",
      confirmDeleteToken: "DELETE",
      confirmDelete: 'Type "{{token}}" to confirm',
      defaultRestored: "Default values restored.",
      saved: "Settings saved.",
      positiveNumbers: "Values must be positive numbers.",
      learnedIntervalBounds:
        "Learned interval must be between {{min}} and {{max}} days.",
      deletedDone: "Deletion complete.",
    },
    stats: {
      title: "Stats",
      subtitle: "Track your progress over 7, 30, or 90 days.",
      overview: "Overview",
      progress: "Progress (reviews)",
      boxSplit: "Box distribution",
      tagsAll: "All tags",
      selectTag: "Select a tag to see details.",
      aggregates: "Aggregates",
      noData: "No data.",
      workOn: "Needs work",
      lowRateHint: "Tags with low success rate (min {{min}} reviews).",
      period7: "7 days",
      period30: "30 days",
      period90: "90 days",
      dueToday: "Due today",
      learned: "Learned",
      reviewsToday: "Reviews today",
      successRate7d: "Success 7d",
      reviews: "Reviews",
      chartLegend: "Green: {{good}}, pink: {{bad}}.",
      rate: "Rate",
      reviewTag: "Review this tag",
      boxDistribution: "Box distribution",
      weakTags: "Weak tags",
      heatmapTitle: "Activity (365 days)",
      heatmapSummary: "{{total}} reviews across {{days}} active days.",
      heatmapLess: "Less",
      heatmapMore: "More",
      reviewsUnit: "reviews",
      dow: {
        mon: "M",
        wed: "W",
        fri: "F",
      },
      retentionTitle: "Retention by box",
      retentionSubtitle: "Success rate of reviews, by starting box.",
      retentionHint:
        "A high rate in box 1 means new cards are mastered quickly.",
    },
    markdown: {
      imageAlt: "Image",
      imageNotFound: "Image not found",
    },
    review: {
      title: "Session",
      empty: "No cards to review.",
      startTraining: "Start training",
      trainingMode: "Training mode (does not change progress).",
      progress: "Progress",
      remaining: "{{count}} cards left",
      cardSize: "Card size",
      revealBack: "Reveal back",
      completed: "Session complete",
      trainingCards: "Training cards",
      noGood: "No correct answers.",
      noBad: "No wrong answers.",
      backHome: "Back to home",
      good: "GOOD",
      bad: "WRONG",
      editCard: "Edit card",
      deleteCard: "Delete card",
      confirmDelete: "Delete this card permanently?",
      confirmDeleteYes: "Delete",
      confirmDeleteNo: "Cancel",
      undo: "Undo last answer",
    },
    packs: {
      title: "Packs",
      subtitle: "Browse and download packs from Supabase.",
      search: "Search",
      descriptionFallback: "No description.",
      cardsCount: "{{count}} card(s)",
      importDirect: "Import",
      importResult: "{{imported}} imported, {{already}} already present",
      importFailed: "Import error: {{message}}",
      download: "Download",
      open: "Open",
      all: "All packs",
      none: "No packs.",
      browseByTags: "Browse public packs by tag folders.",
    },
    importExport: {
      title: "Import / Export",
      subtitle: "Import or export cards as JSON.",
      importTitle: "Import",
      exportTitle: "Export",
      pasteJson: "Paste JSON here",
      exportJson: "Export JSON",
      exportInProgress: "Export in progress...",
      exportDone: "Export complete.",
      importInProgress: "Import in progress...",
      schemaUnsupported: "Unsupported schema version",
      jsonUnsupported: "Unsupported JSON format",
      missingFrontBack: "Card {{index}}: missing front/back",
      duplicateId: "Card {{index}}: duplicate id, new id generated",
      exportPool: "Export pool",
      importPool: "Import pool",
      importSample: "Import sample (debug)",
      summary: "Import summary",
      noValidCards: "No valid cards",
      importNoInsert: "Import with no insert: {{reason}}",
      importDone: "Import complete.",
      importError: "Import error: {{message}}",
      importCsvTsv: "Import CSV/TSV file",
      csvUnsupported: "Invalid CSV/TSV file: front/back columns not found",
      exportAnki: "Export to Anki (.apkg)",
      importAnki: "Import Anki file (.apkg)",
      exportAnkiInProgress: "Exporting to Anki...",
      exportAnkiDone: "Anki export complete.",
      ankiImportDone: "Anki import complete: {{count}} cards imported.",
      ankiImportEmpty: "No valid cards found in the Anki file.",
      ankiImportError: "Error importing Anki file: {{message}}",
    },
    packDetail: {
      title: "Pack",
      slug: "Slug",
      import: "Import this pack into my library",
      importResult: "{{imported}} cards imported, {{already}} already present",
      total: "Total",
      none: "No cards.",
      backToPacks: "Back to packs",
    },
    cardEditor: {
      titleNew: "New card",
      titleEdit: "Edit card",
      front: "Front",
      back: "Back",
      hint: "Hint (optional)",
      tags: "Tags (one tag per line, / for hierarchy)",
      preview: "Preview",
      save: "Save",
      delete: "Delete",
    },
    sharedDeck: {
      import: "Import into my library",
      importResult:
        "{{imported}} card(s) imported, {{already}} already present.",
      importError: "Import error: {{message}}",
      notFound: "This shared deck was not found or has expired.",
      cardCount: "{{count}} card(s)",
      shareTitle: "Share selection",
      shareTitleLabel: "Shared deck title",
      shareConfirm: "Create link",
      shareCancel: "Cancel",
      shareCreating: "Creating...",
      shareLinkLabel: "Share link",
      shareCopy: "Copy",
      shareCopied: "Copied!",
      shareCopyError: "Unable to copy. Please copy manually.",
      shareError: "Share error: {{message}}",
      shareLoginRequired: "You must be signed in to share a deck.",
    },
    notifications: {
      title: "Notifications",
      subtitle: "Daily reminder when cards are due.",
      enable: "Enable reminders",
      permissionDenied: "Permission denied by the browser.",
      permissionGranted: "Notifications enabled.",
    },
  },
} as const;
