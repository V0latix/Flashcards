...
- [ ] Reinitialiser completement l'etat de session quand `tag` ou `mode` change dans `src/routes/ReviewSession.tsx` (reset `index`, `goodCount`, `badCount`, `showBack`, `isDeleteOpen`).
- [ ] Corriger la securite Markdown dans `src/components/MarkdownRenderer.tsx` en retirant ou encadrant `urlTransform={(uri) => uri}`.
